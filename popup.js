...
function setStatus(msg, type="ok"){ const el=qs("status"); if(!el) return; el.textContent=msg; el.className=`small ${type}`; }
...
async function findInterfaceFrameId(tabId){
    ...
    }catch(e){ console.warn("getAllFrames failed, fallback top frame", e); return 0; }
}

async function injectFiles(files){
    const tab = await getActiveTab(); if (!tab?.id) throw new Error("Aucun onglet actif");
...
async function injectSecteurs(){
    try{ const r = await injectFiles(["ct_core.js","secteurs_inspector.js"]); setStatus(`Injecté: core + secteurs (frame ${r.frameId})`,"ok"); }
    catch(e){ console.error(e); setStatus(`Erreur Secteurs: ${e?.message||e}`,"err"); }
}
async function injectColonies(){
    try{ const r = await injectFiles(["ct_core.js","colonies_manager.js"]); setStatus(`Injecté: core + colonies (frame ${r.frameId})`,"ok"); }
    catch(e){ console.error(e); setStatus(`Erreur Colonies: ${e?.message||e}`,"err"); }
}
async function injectResources(){
    try{ const r = await injectFiles(["ct_core.js","resources_bar.js"]); setStatus(`Injecté: core + stocks (frame ${r.frameId})`,"ok"); }
...
// Wire
function wire(){
    const btnLegacy = qs("inject"); if (btnLegacy) btnLegacy.addEventListener("click", injectSecteurs);
